...
with such.A('Region Measure Helpers') as it:
    ...
    def setup():
        ...
        it.excluded_fix = [Fixation(Point(1, 0), 0, 50, it.regions_x[0], excluded=True),
                           Fixation(Point(2, 0), 50, 100, it.regions_x[0], excluded=False),
        ...
        it.fixations_x2 = [Fixation(Point(0, 0), 0, 150, it.regions_x[0]),
                          ...
                          Fixation(Point(11, 0), 650, 700, it.regions_x[1]),
                          Fixation(Point(21, 0), 750, 800, it.regions_x[2]),
                          Fixation(Point(11, 0), 850, 900, it.regions_x[1])]
    ...
    with it.having('first pass fixations'):
        ...
        def test_fp_fixations():
            it.assertEqual(get_fp_fixations(it.trial_x, 0), [it.fixations_x[0], it.fixations_x[1]])
            it.assertEqual(get_fp_fixations(it.trial_y, 3), [it.fixations_y[1], it.fixations_y[2]])
            it.assertEqual(get_fp_fixations(it.trial_x2, 1), [it.fixations_x2[2], it.fixations_x2[3]])
            it.assertEqual(get_fp_fixations(it.trial_x, 2), [it.fixations_x[2]])
