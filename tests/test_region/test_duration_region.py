...
with such.A("Duration Region Measures") as it:
    ...
    with it.having("first_fixation_duration measure"):
        ...
        def test_first_fixation_duration():
            it.assertEqual(
                measures.region.first_fixation_duration(it.trial_x, 2)["value"], 150
            )
            it.assertEqual(
                measures.region.first_fixation_duration(it.trial_y, 1)["value"], 100
            )
            it.assertEqual(
                measures.region.first_fixation_duration(it.trial_x, 0)["value"], 150
            )
            it.assertEqual(
                measures.region.first_fixation_duration(it.trial_y, 3)["value"], 50
            )

        @it.should("be None if the region is skipped")
        def test_first_fixation_duration_skip():
            it.assertEqual(
                measures.region.first_fixation_duration(it.trial_x, 3)["value"], None
            )
            it.assertEqual(
                measures.region.first_fixation_duration(it.trial_y, 2)["value"], None
            )
    ...
    with it.having("single fixation duration measure"):
        ...
        def test_single_fixation():
            it.assertEqual(
                measures.region.single_fixation_duration(it.trial_x, 2)["value"], 150
            )
            it.assertEqual(
                measures.region.single_fixation_duration(it.trial_y, 1)["value"], 100
            )
            it.assertEqual(
                measures.region.single_fixation_duration(it.trial_x, 0)["value"], None
            )
            it.assertEqual(
                measures.region.single_fixation_duration(it.trial_y, 3)["value"], None
            )

        @it.should("be None if the region is skipped in first pass")
        def test_single_fixation_skip():
            it.assertEqual(
                measures.region.single_fixation_duration(it.trial_x, 3)["value"], None
            )
            it.assertEqual(
                measures.region.single_fixation_duration(it.trial_y, 2)["value"], None
            )
            it.assertEqual(
                measures.region.single_fixation_duration(it.trial_x, 1)["value"], None
            )
            it.assertEqual(
                measures.region.single_fixation_duration(it.trial_y, 0)["value"], None
            )
    ...
    with it.having("right-bounded time measure"):
        ...
        def test_right_bounded_time_skip():
            ...
            it.assertEqual(
                measures.region.right_bounded_time(it.trial_y, 0)["value"], None
            )
    ...
    with it.having("refixation time measure"):
        ...
        def test_refixation_time_skip():
            it.assertEqual(
                measures.region.refixation_time(it.trial_x, 3)["value"], None
            )
            it.assertEqual(
                measures.region.refixation_time(it.trial_y, 2)["value"], None
            )
    ...
    with it.having("go back time by character measure"):
        ...
        @it.should("calculate go back time correctly")
        def test_go_back_time_char():
            it.assertEqual(
                measures.region.go_back_time_char(it.trial_x2, 0)["value"], 370
            )
            it.assertEqual(
                measures.region.go_back_time_char(it.trial_x, 0)["value"], 350
            )
            it.assertEqual(
                measures.region.go_back_time_char(it.trial_y, 3)["value"], 250
            )

        @it.should("measures from the previous fixation if the region is skipped")
        def test_refixation_char_time_skip():
            it.assertEqual(
                measures.region.go_back_time_char(it.trial_x2, 1)["value"], 170
            )
    ...
    with it.having("go back time by region measure"):
        ...
        def test_go_back_time_region():
            it.assertEqual(
                measures.region.go_back_time_region(it.trial_x, 0)["value"], 350
            )
            it.assertEqual(
                measures.region.go_back_time_region(it.trial_y, 3)["value"], 250
            )

        @it.should("measures from the previous fixation if the region is skipped")
        def test_go_back_time_region_skip():
            it.assertEqual(
                measures.region.go_back_time_region(it.trial_y, 2)["value"], 250
            )
