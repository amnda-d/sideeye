...
with such.A("Boolean Region Measures") as it:
    ...
    with it.having("skip measure"):
        ...
        @it.should(
            """be true if there is a fixation in a future region before
    ...
    with it.having("first pass regressions out measure"):
        ...
        def test_regressions_out_true():
            it.assertTrue(
                measures.region.first_pass_regressions_out(it.trial_x, 2)["value"]
            )
            it.assertTrue(
                measures.region.first_pass_regressions_out(it.trial_y, 3)["value"]
            )

        @it.should("be false if there is not a regression out of the target region")
        def test_regressions_out_false():
            it.assertFalse(
                measures.region.first_pass_regressions_out(it.trial_x, 0)["value"]
            )
            it.assertFalse(
                measures.region.first_pass_regressions_out(it.trial_y, 1)["value"]
            )

        @it.should("be None if the region is never fixated")
        def test_regressions_out_none():
            it.assertEqual(
                measures.region.first_pass_regressions_out(it.trial_x, 3)["value"], None
            )
            it.assertEqual(
                measures.region.first_pass_regressions_out(it.trial_y, 2)["value"], None
            )
        ...
        def test_regressions_out_exclude():
            it.assertFalse(
                measures.region.first_pass_regressions_out(it.excluded_trial, 1)[
                    "value"
                ]
            )
    ...
    with it.having("first pass regressions in measure"):
        ...
        def test_regressions_in_true():
            it.assertTrue(
                measures.region.first_pass_regressions_in(it.trial_x, 1)["value"]
            )
            it.assertTrue(
                measures.region.first_pass_regressions_in(it.trial_y, 0)["value"]
            )

        @it.should("be false if there is not a regression into the target region")
        def test_regressions_in_false():
            it.assertFalse(
                measures.region.first_pass_regressions_in(it.trial_x, 2)["value"]
            )
            it.assertFalse(
                measures.region.first_pass_regressions_in(it.trial_y, 3)["value"]
            )

        @it.should("be None if the region is never fixated")
        def test_regressions_in_none():
            it.assertEqual(
                measures.region.first_pass_regressions_in(it.trial_x, 3)["value"], None
            )
            it.assertEqual(
                measures.region.first_pass_regressions_in(it.trial_y, 2)["value"], None
            )
        ...
        def test_regressions_in_exclude():
            it.assertFalse(
                measures.region.first_pass_regressions_in(it.excluded_trial, 0)["value"]
            )
