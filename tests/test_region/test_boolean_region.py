...
with such.A('Boolean Region Measures') as it:
    ...
    def setup():
        ...
        it.excluded_fix = [Fixation(Point(1, 0), 0, 100, it.regions_x[0], excluded=False),
    ...
    with it.having('skip measure'):
        @it.should('be true if there is a fixation in a future region before there is a fixation in the target region')
    ...
    with it.having('first pass regressions out measure'):
        ...
        def test_regressions_out_exclude():
            it.assertFalse(measures.region.first_pass_regressions_out(it.excluded_trial, 1)['value'])
