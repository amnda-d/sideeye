...
with such.A("Other Region Measures") as it:
    ...
    with it.having("landing position measure"):
        ...
        def test_landing_position():
            it.assertEqual(
                measures.region.landing_position(it.trial_x, 2)["value"], '"(2, 0)"'
            )
            it.assertEqual(
                measures.region.landing_position(it.trial_y, 1)["value"], '"(2, 0)"'
            )
            it.assertEqual(
                measures.region.landing_position(it.trial_x, 0)["value"], '"(0, 0)"'
            )
            it.assertEqual(
                measures.region.landing_position(it.trial_y, 3)["value"], '"(2, 0)"'
            )

        @it.should("be None if the region is skipped")
        def test_landing_position_skip():
            it.assertEqual(
                measures.region.landing_position(it.trial_x, 3)["value"], None
            )
            it.assertEqual(
                measures.region.landing_position(it.trial_y, 2)["value"], None
            )
    ...
    with it.having("launch site measure"):
        ...
        def test_launch_site():
            it.assertEqual(
                measures.region.launch_site(it.trial_x, 2)["value"], '"(-17, 0)"'
            )
            it.assertEqual(
                measures.region.launch_site(it.trial_y, 3)["value"], '"(2, -1)"'
            )
    ...
    with it.having("first pass fixation count measure"):
        ...
        def test_fixation_count():
            it.assertEqual(
                measures.region.first_pass_fixation_count(it.trial_x, 2)["value"], 1
            )
            it.assertEqual(
                measures.region.first_pass_fixation_count(it.trial_y, 1)["value"], 1
            )
            it.assertEqual(
                measures.region.first_pass_fixation_count(it.trial_x, 0)["value"], 2
            )
            it.assertEqual(
                measures.region.first_pass_fixation_count(it.trial_y, 3)["value"], 2
            )

        @it.should("be None if the region is skipped")
        def test_fixation_count_skip():
            it.assertEqual(
                measures.region.first_pass_fixation_count(it.trial_x, 3)["value"], None
            )
            it.assertEqual(
                measures.region.first_pass_fixation_count(it.trial_y, 2)["value"], None
            )
