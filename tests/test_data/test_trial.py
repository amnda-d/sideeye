...
with such.A("Trial") as it:
    ...
    def setup():
        ...
        it.trial_include_fixations = Trial(
            1, 5, it.item, it.fixations, include_fixation=True
        )
        it.trial_include_saccades = Trial(
            1, 5, it.item, it.fixations, include_saccades=True
        )
    ...
    @it.should(
        "calculate saccades correctly when include_saccades and include_fixation are True"
    )
    ...
    def test_trial_equality():
        it.assertTrue(
            Trial(1, 5, it.item, it.fixations) == Trial(1, 5, it.item, it.fixations)
        )
        it.assertTrue(
            Trial(1, 5, it.item, it.fixations)
            != Trial(1, 5, it.item, [it.fix1, it.fix2])
        )
