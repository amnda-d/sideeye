...
with such.A('Trial Measures') as it:
    ...
    def setup():
        ...
        it.no_regressions_fix = [Fixation(Point(0, 0), 0, 100, it.regions_x[0]),
    ...
    with it.having('location of first regression measure'):
        @it.should('return the correct position of the last fixation before the first regression in a trial.')
    ...
    with it.having('latency to first regression measure'):
        @it.should('return the correct time until the end of the last fixation before the first regression in a trial.')
