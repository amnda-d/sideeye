...
with such.A("Trial Measures") as it:
    ...
    def setup():
        ...
        it.fixations_x = [
            ...
            Fixation(Point(15, 0), 400, 450, 4, it.regions_x[1], excluded=True),
        ]
    ...
    with it.having("location of first regression measure"):
        ...
        @it.should(
            """return the correct position of the last fixation before the
                      first regression in a trial."""
        )
        def test_location_first_regression():
            it.assertEqual(
                measures.trial.location_first_regression(it.trial_x), '"(22, 0)"'
            )
            it.assertEqual(
                measures.trial.location_first_regression(it.trial_y), '"(15, 1)"'
            )
            it.assertEqual(
                measures.trial.location_first_regression(it.no_regressions), None
            )

    with it.having("latency to first regression measure"):

        @it.should(
            """return the correct time until the end of the last fixation
        ...
        def test_latency_first_regression():
            ...
            it.assertEqual(
                measures.trial.latency_first_regression(it.no_regressions), None
            )
    ...
    with it.having("average forward saccade length measure"):
        ...
        def test_average_forward_saccade():
            ...
            it.assertEqual(
                measures.trial.average_forward_saccade(it.no_regressions), 20
            )
    ...
    with it.having("average backward saccade length measure"):
        ...
        def test_average_backward_saccade():
            ...
            it.assertEqual(
                measures.trial.average_backward_saccade(it.no_regressions), 0
            )
