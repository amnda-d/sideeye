...
While the calculations and outputs generated by SideEye can be customized, SideEye can be used easily without additional configuration. After installing SideEye, open a new .py file. Save this file in a directory containing a region file and a directory of .DA1 files. The directory structure should look like this:
...
::
  ...
  # Parse DA1 files into a list of Experiment objects.
  experiments = sideeye.parser.experiment.parse_dir('DA1_files', 'region_file.cnt', 'config.json')

  # Calculate all measures and output results as a .csv file.
  sideeye.calculate_all_measures(experiments, 'output.csv', 'config.json')
...
::
  ...
  # Parse DA1 files into a list of Experiment objects.
  experiments = sideeye.parser.experiment.parse_dir('DA1_files', 'region_file.cnt')
